...
class LayoutManager {
  ...
  constructor(layoutname) {
    ...
    if (path.extname(layoutname) !== LAYOUT_EXTENSION)
      layoutname += LAYOUT_EXTENSION
  ...
  loadFromFile() {
    if (fs.existsSync(this.layoutFilename)) {
      try {
        ...
        logger.info(`[Layout] Loaded${cmdline.debug ? ` from ${this.layoutFilename}` : ''}`)
      }
      catch (error) {
