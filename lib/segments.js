...
class Segment extends EventEmitter {
  ...
  InitAllEvents() {
    this.turnout1.on('change', (...arguments_) => this.emit('event', this.segmentid, 'turnout1', 'change', ...arguments_))
    this.turnout2.on('change', (...arguments_) => this.emit('event', this.segmentid, 'turnout2', 'change', ...arguments_))
    this.turnout3.on('change', (...arguments_) => this.emit('event', this.segmentid, 'turnout3', 'change', ...arguments_))
    this.turnout4.on('change', (...arguments_) => this.emit('event', this.segmentid, 'turnout4', 'change', ...arguments_))

    this.signal1.on('change', (...arguments_) => this.emit('event', this.segmentid, 'signal1', 'change', ...arguments_))
    this.signal2.on('change', (...arguments_) => this.emit('event', this.segmentid, 'signal2', 'change', ...arguments_))
    this.signal3.on('change', (...arguments_) => this.emit('event', this.segmentid, 'signal3', 'change', ...arguments_))
    this.signal4.on('change', (...arguments_) => this.emit('event', this.segmentid, 'signal4', 'change', ...arguments_))

    this.keypad.on('switchchange', (...arguments_) => this.emit('event', this.segmentid, 'keypad', 'switchchange', ...arguments_))
    this.keypad.on('buttonpress', (...arguments_) => this.emit('event', this.segmentid, 'keypad', 'buttonpress', ...arguments_))
    this.keypad.on('buttonshortpress', (...arguments_) => this.emit('event', this.segmentid, 'keypad', 'buttonshortpress', ...arguments_))
    this.keypad.on('buttonlongpress', (...arguments_) => this.emit('event', this.segmentid, 'keypad', 'buttonlongpress', ...arguments_))

    this.sound.on('change', (...arguments_) => this.emit('event', this.segmentid, 'sound', 'change', ...arguments_))

    this.ambientlight1.on('nightchange', (...arguments_) => this.emit('event', this.segmentid, 'ambientlight1', 'nightchange', ...arguments_))
    this.ambientlight1.on('lightchange', (...arguments_) => this.emit('event', this.segmentid, 'ambientlight1', 'lightchange', ...arguments_))
    this.ambientlight1.on('effectchange', (...arguments_) => this.emit('event', this.segmentid, 'ambientlight1', 'effectchange', ...arguments_))
    this.ambientlight2.on('nightchange', (...arguments_) => this.emit('event', this.segmentid, 'ambientlight2', 'nightchange', ...arguments_))
    this.ambientlight2.on('lightchange', (...arguments_) => this.emit('event', this.segmentid, 'ambientlight2', 'lightchange', ...arguments_))
    this.ambientlight2.on('effectchange', (...arguments_) => this.emit('event', this.segmentid, 'ambientlight2', 'effectchange', ...arguments_))
    this.ambientlight3.on('nightchange', (...arguments_) => this.emit('event', this.segmentid, 'ambientlight3', 'nightchange', ...arguments_))
    this.ambientlight3.on('lightchange', (...arguments_) => this.emit('event', this.segmentid, 'ambientlight3', 'lightchange', ...arguments_))
    this.ambientlight3.on('effectchange', (...arguments_) => this.emit('event', this.segmentid, 'ambientlight3', 'effectchange', ...arguments_))
    this.ambientlight4.on('nightchange', (...arguments_) => this.emit('event', this.segmentid, 'ambientlight4', 'nightchange', ...arguments_))
    this.ambientlight4.on('lightchange', (...arguments_) => this.emit('event', this.segmentid, 'ambientlight4', 'lightchange', ...arguments_))
    this.ambientlight4.on('effectchange', (...arguments_) => this.emit('event', this.segmentid, 'ambientlight4', 'effectchange', ...arguments_))
  }

  constructor(segmentid = 0) {
    super()
    this.InitAllEvents()
    this.segmentid = segmentid
  }
  IsValid() { return this.segmentid ? true : false }
  ...
  GetMode() {
    switch (this.mode) {
      ...
      default:
        return { mode: 'unknown', id: this.segmentid }
    }
  }
...
class SegmentsHandler extends EventEmitter {
  ...
  constructor(rf24handler, wifihandler) {
    ...
    rf24handler.on('receive', function (fromNode, dataBuffer) {
      this.GetSegmentById(fromNode).ReceiveRf24(dataBuffer)
    }.bind(this))
    wifihandler.on('receive', function (fromNode, ip, dataBuffer) {
      this.GetSegmentById(fromNode).ReceiveWiFi(ip, dataBuffer)
    }.bind(this))
  }
