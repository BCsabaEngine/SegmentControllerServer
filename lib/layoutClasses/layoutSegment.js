...
const { Grid, GridItem } = require('./grid')
...
class LayoutSegment {
  ...
  drawTerrains(context, layout, x, y) {
    ...
    class TerrainGridItem extends GridItem {
      onInitItem(element) {
      ...
      draw(context, layout, x, y, grid) {
        const bs = layout.blockSize

        context.fillStyle = this.color
        context.fillRect((this.x + x) * bs, (this.y + y) * bs, bs, bs)
      }
    }
    ...
    for (const grid of grids) {
      ...
      //   context.beginPath()
      //   context.lineWidth = 1
      //   context.strokeStyle = Color(line.item.color).lighten(0.2).hex()
      ...
      for (const item of grid.getItems())
        item.draw(context, layout, x, y, grid)
    }
  }
