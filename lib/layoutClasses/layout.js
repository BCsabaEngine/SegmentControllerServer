...
const LayoutRuleCode = require('./layoutRuleCode')
...
class Layout {
  ...
  addNewRuleCode(newname) {
    for (const rulecode of this.rulecodes)
      if (rulecode.name === newname)
        throw new Error(`Rulecode with name ${newname} already exists`)

    const rc = new LayoutRuleCode(newname)
    this.rulecodes.push(rc)

    return rc.id
  }
  renameRuleCode(id, newname) {
    for (const rulecode of this.rulecodes)
      if (rulecode.name === newname && rulecode.id !== id)
        throw new Error(`Rulecode with name ${newname} already exists`)

    for (const rulecode of this.rulecodes)
      if (rulecode.id === id) {
        rulecode.name = newname
        break
      }
  }
  updateRuleCode(id, jscode) {
    for (const rulecode of this.rulecodes)
      if (rulecode.id === id) {
        rulecode.jscode = jscode
        break
      }
  }
  deleteRuleCode(id) {
    this.rulescodes = this.rulescodes.filter(item => item.id !== id)
  }
}
