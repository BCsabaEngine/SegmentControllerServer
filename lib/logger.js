...
const logger = winston.createLogger({
  ...
  format: cmdline.debug ?
    winston.format.printf(({ level, message, label, timestamp }) => `${dayjs(timestamp).format('HH:mm:ss')} [${level}] ${message}`)
    :
    winston.format.printf(({ level, message, label, timestamp }) => `${level !== 'info' ? `[${level}] ` : ''}${message}`),
  transports: [
...
setInterval(logger.removeOldLogs, autoRemoveOldLogsHours * 60 * 60 * 1000)
logger.debug(`Archiving scheduled: every ${autoRemoveOldLogsHours}h`)
